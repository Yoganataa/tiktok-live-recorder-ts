...
const FREE_MAX_SIZE = 2 * 1024 * 1024 * 1024;
const PREMIUM_MAX_SIZE = 4 * 1024 * 1024 * 1024;
export class TelegramUploader {
  ...
  private async getClient(): Promise<TelegramClient> {
    ...
    await client.start({
      phoneNumber: async () => {
        throw new Error(
          'Interactive login is not supported. Please set TELEGRAM_SESSION in your .env file.',
        );
      },
      password: async () => {
        throw new Error(
          '2FA Password required but interactive mode is disabled. Please update your session string.',
        );
      },
      phoneCode: async () => {
        throw new Error(
          'Login code required but interactive mode is disabled. Please update your session string.',
        );
      },
      onError: (err) => {
